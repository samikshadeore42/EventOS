...
function PortalSkeleton() {
  return (
    <div className="max-w-4xl mx-auto px-4 py-12">
      <div className="h-6 w-48 bg-cardSoft rounded animate-pulse mb-2" />
      <div className="h-4 w-32 bg-cardSoft rounded animate-pulse mb-8" />
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
        {[1,2,3,4].map(i => <div key={i} className="h-24 bg-cardSoft rounded-xl animate-pulse" />)}
      </div>
      <div className="h-64 bg-cardSoft rounded-xl animate-pulse" />
    </div>
  )
}
        ...
        <div className="flex justify-between items-start mb-3">
          <div>
            ...
            <div className="flex items-baseline gap-2">
              ...
              {trend && <span className={`text-sm font-bold ${trend > 0 ? 'text-emerald-500' : 'text-primary'}`}>{trend > 0 ? '+' : ''}{trend}%</span>}
            </div>
          </div>
          {Icon && (
...
function ScheduleMeetingForm({ teamId, token, onSuccess }) {
  ...
  return (
    <div className="glass-card rounded-2xl border-l-2 border-l-primary shadow-sm p-5 relative overflow-hidden group">
      ...
      <div className="relative z-10">
        <h3 className="text-sm font-bold text-foreground mb-4 flex items-center gap-2">
          <Calendar size={14} className="text-primary" /> Schedule Meeting
        </h3>
        <div className="grid grid-cols-2 gap-3 mb-3">
          <div>
            <label className="block text-xs font-bold text-muted mb-1">Title</label>
            <input value={form.title} onChange={e => setForm(f => ({...f, title: e.target.value}))}
              placeholder="Daily standup" className="w-full bg-background text-foreground border border-border rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-1 focus:ring-primary focus:border-primary shadow-sm placeholder-muted" />
          </div>
          <div>
            ...
            <input value={form.meeting_url} onChange={e => setForm(f => ({...f, meeting_url: e.target.value}))}
              placeholder="https://meet.google.com/..." className="w-full bg-background text-foreground border border-border rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-1 focus:ring-primary focus:border-primary shadow-sm placeholder-muted" />
          </div>
          <div>
            <label className="block text-xs font-bold text-muted mb-1">Date & Time</label>
            <input type="datetime-local" value={form.scheduled_at} onChange={e => setForm(f => ({...f, scheduled_at: e.target.value}))}
              className="w-full bg-background text-foreground border border-border rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-1 focus:ring-primary focus:border-primary shadow-sm" />
          </div>
          <div>
        ...
        <div className="mb-3">
          ...
          <textarea value={form.agenda} onChange={e => setForm(f => ({...f, agenda: e.target.value}))}
            rows={2} placeholder="Topics to discuss..." className="w-full bg-background text-foreground border border-border rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-1 focus:ring-primary focus:border-primary resize-none shadow-sm placeholder-muted" />
        </div>
        <div className="flex justify-end">
          <button onClick={() => mutation.mutate()} disabled={mutation.isPending || !form.title || !form.meeting_url || !form.scheduled_at}
            className="flex items-center gap-1.5 text-sm font-semibold px-4 py-2 rounded-lg btn-primary text-white hover:bg-primary-dark disabled:opacity-50 disabled:cursor-not-allowed shadow-sm transition-all">
            {mutation.isPending ? <Loader2 size={14} className="animate-spin" /> : <Plus size={14} />} Schedule
          </button>
        </div>
        {mutation.isError && <p className="mt-2 text-xs font-semibold text-primary">{mutation.error?.message}</p>}
        {mutation.isSuccess && <p className="mt-2 text-xs font-semibold text-primary">Meeting scheduled successfully!</p>}
      </div>
    </div>
  )
}
      ...
      <div className="relative z-10">
        ...
        <div className="mb-3">
          ...
          <textarea value={form.feedback_text} onChange={e => setForm(f => ({...f, feedback_text: e.target.value}))}
            rows={3} placeholder="Observations, progress notes..." className="w-full bg-background text-foreground border border-border rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-[#C84BEA] focus:border-[#C84BEA] resize-none shadow-sm placeholder-muted" />
        </div>
        ...
        <div className="grid grid-cols-2 gap-3 mb-3">
          <div>
            ...
            <textarea value={form.blockers} onChange={e => setForm(f => ({...f, blockers: e.target.value}))}
              rows={2} placeholder="Any blockers..." className="w-full bg-background text-foreground border border-border rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-[#C84BEA] focus:border-[#C84BEA] resize-none shadow-sm placeholder-muted" />
          </div>
          <div>
            ...
            <textarea value={form.action_items_str} onChange={e => setForm(f => ({...f, action_items_str: e.target.value}))}
              rows={2} placeholder="Complete API integration&#10;Fix auth bug" className="w-full bg-background text-foreground border border-border rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-[#C84BEA] focus:border-[#C84BEA] resize-none shadow-sm placeholder-muted" />
          </div>
        </div>
        ...
        {mutation.isError && <p className="mt-2 text-xs font-semibold text-primary">{mutation.error?.message}</p>}
        {mutation.isSuccess && <p className="mt-2 text-xs font-semibold text-primary">Feedback submitted!</p>}
      </div>
    </div>
  )
}
...
// ── Workspace Card ───────────────────────────────────────────────────────────
function WorkspaceCard({ title, icon: Icon, mainText, subText, actionText, onAction, colorClass = "text-primary bg-cardSoft border-border dark:bg-primary/10" }) {
  return (
...
function TeamCard({ team, token, eventId, mentorId }) {
          ...
          <div className="w-full max-h-[85vh] overflow-y-auto custom-scrollbar shadow-2xl rounded-2xl">
                ...
                {team.members?.map((m, i) => (
                  <div key={m.id} className="flex items-center gap-3 bg-surface border border-border rounded-lg p-3">
                    <div className={`w-10 h-10 rounded-full flex items-center justify-center text-sm font-bold shrink-0 ${
                      ['bg-cardSoft border border-border text-primary-dark', 'bg-cardSoft border border-border text-primary-dark', 'bg-cardSoft border border-border text-primary-dark', 'bg-cardSoft border border-border text-primary-dark'][i % 4]
                    }`}>{initials(m.name)}</div>
            ...
            {activeModal === 'progress' && (
              <div className="text-center py-12">
                <Target size={32} className="mx-auto mb-3 text-muted" />
  ...
  return (
    <div className="glass-card rounded-2xl border-l-2 border-l-primary shadow-sm overflow-hidden relative group transition-all hover:-translate-y-1 hover:scale-[1.01]">
      <div className="absolute -right-8 -top-8 w-40 h-40 bg-gradient-to-br from-primary/10 to-transparent rounded-full blur-3xl group-hover:scale-125 transition-transform duration-700 pointer-events-none" />
      <div className="relative z-10">
        <div className="flex items-center gap-3 px-4 py-4 cursor-pointer hover:bg-surface/50 transition-colors" onClick={() => setExpanded(!expanded)}>
          <div className="w-12 h-12 rounded-xl bg-cardSoft border border-border text-primary-dark flex items-center justify-center font-bold text-sm shrink-0">
          ...
          <button className="px-4 py-2 rounded-lg bg-surface border border-border text-xs font-bold text-primary hover:bg-cardSoft hover:border-border transition-colors">
            Manage Team
          </button>
        </div>
        ...
        {expanded && createPortal(
          <div className="fixed inset-0 z-[90] flex items-center justify-center p-4 sm:p-6 md:p-12 bg-black/55 backdrop-blur-[8px]">
            <div className="bg-card rounded-2xl w-full max-w-4xl shadow-2xl flex flex-col max-h-full shrink min-h-0 relative overflow-hidden border border-border">
              {/* Workspace Header */}
              ...
              <div className="flex flex-col sm:flex-row flex-1 min-h-0 overflow-hidden">
                 ...
                 {/* Left Column: Actions */}
                 <div className="flex-1 p-5 sm:p-6 overflow-y-auto custom-scrollbar bg-cardSoft sm:border-r border-border">
                    <div className="grid grid-cols-1 gap-4">
                      {/* Schedule Meeting */}
                      <WorkspaceCard
                        title="Schedule Meet"
                        icon={Calendar}
                        colorClass="text-primary bg-cardSoft border-border dark:bg-primary/10"
      ...
      <div className="min-h-screen flex items-center justify-center bg-surface px-4">
        <div className="text-center max-w-sm">
          ...
          <p className="text-sm font-medium text-muted">
            ...
            It looks like <code className="text-xs bg-cardSoft text-foreground px-1 py-0.5 rounded border border-border">/mentor?token=…</code>
          </p>
        </div>
      </div>
    )
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-surface px-4">
        <div className="text-center max-w-sm">
          ...
          <p className="text-sm font-medium text-muted">
            This link is for mentors. Your token has role <code className="bg-cardSoft border border-border px-1 rounded text-xs">{profileData.role}</code>.
          </p>
        </div>
      </div>
    )
  ...
  return (
    <AppLayout title="WiSE@TI Hackathon" subtitle="Mentor Portal" userName={profile.name}>
      <div className="max-w-4xl mx-auto px-4 py-4">
        {/* Header */}
        <div className="mb-8">
          <p className="text-xs font-bold text-primary uppercase tracking-widest mb-1">Mentor Portal</p>
        ...
        <div className="mb-4">
          <h2 className="text-base font-bold text-foreground mb-4 flex items-center gap-2">
            <Users size={16} className="text-primary" /> Your Teams
          </h2>

          {teamsLoading ? (
            <div className="space-y-3">
              {[1,2].map(i => <div key={i} className="h-16 bg-cardSoft rounded-xl animate-pulse" />)}
            </div>
          ) : teams.length === 0 ? (
