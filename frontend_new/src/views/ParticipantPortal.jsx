...
function PortalHeader({ name, email, eventName, stage }) {
  return (
    <div className="text-center mb-10">
      ...
      {stage && (
        <div className="inline-flex items-center gap-1.5 mt-3 px-3 py-1 rounded-full bg-indigo-500/10 border border-indigo-500/20 border border-indigo-500/20 text-xs font-medium text-indigo-300">
          <span className="w-1.5 h-1.5 rounded-full bg-indigo-500/10 border border-indigo-500/200 animate-pulse" />
...
function EventTimeline({ timeline }) {
  ...
  return (
    <div className="glass-panel rounded-2xl border border-white/10 p-6 mb-6">
      <h2 className="text-sm font-semibold text-gray-600 mb-5">Your Event Journey</h2>
            ...
            return (
              <div key={index} className="flex items-start gap-4 relative">
                ...
                <div className={`relative z-10 w-8 h-8 rounded-full border-2 flex items-center justify-center shrink-0 ${
                  isCompleted ? 'bg-teal-500 shadow-[0_0_10px_rgba(20,184,166,0.5)]/10 border border-teal-500/200  border-teal-500'   :
                  isActive    ? 'bg-indigo-600 shadow-[0_0_15px_rgba(79,70,229,0.4)] hover:shadow-[0_0_25px_rgba(79,70,229,0.6)] border-indigo-600' :
                ...
                <div className={`flex-1 pt-1 pb-1 ${isPending ? 'opacity-40' : ''}`}>
                  <div className="flex items-center gap-2">
                    <p className={`text-sm font-semibold ${
                      ...
                      isCompleted ? 'text-teal-300'   : 'text-gray-500'
                    }`}>
                      {phase.phase}
                    </p>
                    {isActive && (
                      <span className="text-xs font-medium text-white bg-indigo-500/10 border border-indigo-500/200 px-2 py-0.5 rounded-full">
                        In progress
                      </span>
                    )}
                    {isCompleted && (
                      <span className="text-xs font-medium text-teal-400 bg-teal-500 shadow-[0_0_10px_rgba(20,184,166,0.5)]/10 border border-teal-500/20 px-2 py-0.5 rounded-full">
                        Complete
                      </span>
                    )}
                  </div>
  ...
  return (
    <div className="flex items-center gap-3 bg-white/5 rounded-2xl p-3 border border-white/5">
      ...
      <div className="min-w-0">
        ...
        <p className="text-xs text-gray-500 truncate">{teammate.institution}</p>
      </div>
    </div>
  )
}
...
function TeamRevealSection({ teamName, rationale, teammates }) {
  ...
  return (
    <div className="mb-6">
      ...
      {rationale && (
        <div className="glass-panel rounded-2xl border border-white/10 mb-4 overflow-hidden">
          <button
            onClick={() => setRationaleOpen((o) => !o)}
            className="w-full flex items-center justify-between px-5 py-4 hover:bg-white/5 transition-colors"
          >
            <div className="flex items-center gap-2 text-left">
              <span className="text-sm font-semibold text-gray-200">Why was this team formed?</span>
              <span className="text-xs text-indigo-500 bg-indigo-500/10 border border-indigo-500/20 px-2 py-0.5 rounded-full">AI analysis</span>
            </div>
            {rationaleOpen
          ...
          {rationaleOpen && (
            <div className="px-5 pb-5 border-t border-white/5">
              <p className="text-sm text-gray-500 leading-relaxed pt-4">{rationale}</p>
            </div>
          )}
        </div>
      )}
...
function AwaitingCard() {
  return (
    <div className="bg-amber-500/10 border border-amber-500/20 border border-amber-200 rounded-2xl p-6 text-center mb-6">
  ...
  return (
    <div className="glass-panel rounded-2xl border border-white/10 p-5 mb-6">
      <div className="flex items-center gap-2 mb-4">
        ...
        <h3 className="text-sm font-semibold text-gray-200">Key Dates</h3>
      </div>
        ...
        {dates.map((d, i) => (
          <div key={i} className="flex items-center justify-between text-sm">
            <div className="flex items-center gap-2">
              ...
              <span className={d.done ? 'text-gray-600' : 'text-gray-500'}>{d.label}</span>
            </div>
            <span className={`text-xs font-medium ${d.done ? 'text-teal-400' : 'text-gray-600'}`}>
              {d.date}
            </span>
          </div>
        ))}
...
function SupportFooter({ supportEmail }) {
  return (
    <div className="text-center pt-4 pb-10">
      ...
      <p className="text-xs text-gray-600 mt-1">EventOS · WiSE@TI Hackathon</p>
    </div>
  )
}
  ...
  return (
    <div className="max-w-lg mx-auto px-4 py-12">
      <div className="text-center mb-10">
        ...
        <div className="h-8 w-56 bg-white/10 rounded animate-pulse mx-auto mb-2" />
      ...
      <div className="glass-panel rounded-2xl border border-white/10 p-6 mb-4">
        <div className="h-4 w-32 bg-white/10 rounded animate-pulse mb-5" />
        {[1,2,3,4].map(i => (
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-white/5 px-4">
        <div className="text-center max-w-sm">
          ...
          <h2 className="text-lg font-bold text-gray-600 mb-1">No access token</h2>
          <p className="text-sm text-gray-500">
            Please use the secure link sent to your email.
            It looks like <code className="text-xs bg-white/10 px-1 py-0.5 rounded">/portal?token=…</code>
          </p>
        </div>
      </div>
    )
  ...
  if (data && data.evaluator_id) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-white/5 px-4">
        <div className="text-center max-w-sm">
          <AlertTriangle size={40} className="text-amber-400 mx-auto mb-4" />
          <h2 className="text-lg font-bold text-gray-600 mb-1">Wrong portal</h2>
          <p className="text-sm text-gray-500">
            This link is for participants. Judges should visit{' '}
            <code className="text-xs bg-white/10 px-1 py-0.5 rounded">/judge?token=…</code>
          </p>
        </div>
      </div>
    )
