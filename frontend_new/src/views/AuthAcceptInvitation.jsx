  ...
  const renderContent = () => {
    switch (status) {
      case 'loading':
        return (
          <div className="text-center">
            <Loader2 className="animate-spin h-12 w-12 text-warning mx-auto mb-4" />
      ...
      case 'ready':
        return (
          <div className="text-center">
            <div className="mx-auto flex items-center justify-center h-14 w-14 rounded-full bg-cardSoft dark:bg-amber-900/50 mb-4">
              <Building className="h-7 w-7 shrink-0" />
            </div>
            <h3 className="text-lg font-medium text-foreground mb-2">
              You&apos;re invited to join
            </h3>
            <p className="text-xl font-bold shrink-0 mb-1">{preview?.organization_name}</p>
            ...
            {error && (
              <div className="bg-cardSoft border border-border shrink-0 p-3 rounded-lg text-sm text-center mb-4">
                {error}
              </div>
            )}
            ...
            {!showCreateAccount ? (
              <>
                ...
                {!authenticated && preview?.has_account && (
                  <p className="text-xs text-muted mt-3">
                    ...
                    <button onClick={() => setShowCreateAccount(true)} className="shrink-0 hover:underline">
              ...
              <div className="space-y-3 text-left">
                ...
                <button
                  ...
                  className="w-full flex justify-center py-2.5 px-4 rounded-lg shadow-lg shadow-amber-500/25 text-sm font-medium text-white app-btn-primary disabled:opacity-100 disabled:bg-cardSoft dark:disabled:bg-amber-900/50 disabled:text-amber-400 dark:disabled:text-primary disabled:border-transparent disabled:shadow-none disabled:cursor-not-allowed"
                >
      ...
      case 'accepted':
        return (
          <div className="text-center">
            ...
            <Link to="/admin" className="inline-flex items-center px-4 py-2 rounded-lg text-sm font-medium text-white app-btn-primary">
              Go to Dashboard
            </Link>
          </div>
        );
      case 'expired':
        return (
          <div className="text-center">
            ...
            <Link to="/auth/login" className="font-medium shrink-0 hover:text-warning">Go to sign in</Link>
          </div>
        );
      default:
        return (
