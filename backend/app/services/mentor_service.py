...
class MentorService:
    """Stateless service — all methods take a db session as first arg."""
    ...
    def create_mentor(db: Session, data: MentorCreate) -> Mentor:
        ...
        if existing:
            raise HTTPException(status_code=409, detail=f"Mentor with email {data.email} already exists.")
        mentor = Mentor(
    ...
    @staticmethod
    def list_mentors(db: Session, active_only: bool = False) -> list[dict]:
    ...
    @staticmethod
    def get_mentor(db: Session, mentor_id: UUID) -> Mentor:
    ...
    @staticmethod
    def update_mentor(db: Session, mentor_id: UUID, data: MentorUpdate) -> Mentor:
    ...
    @staticmethod
    def deactivate_mentor(db: Session, mentor_id: UUID) -> dict:
        mentor = MentorService.get_mentor(db, mentor_id)
    ...
    @staticmethod
    def assign_mentor_to_team(db: Session, data: MentorAssignmentCreate) -> MentorAssignment:
        mentor = MentorService.get_mentor(db, data.mentor_id)
        ...
        # Deactivate any existing active assignment for this team
        db.query(MentorAssignment).filter(
    ...
    @staticmethod
    def unassign_mentor_from_team(db: Session, assignment_id: UUID) -> dict:
        assignment = db.query(MentorAssignment).filter(MentorAssignment.id == assignment_id).first()
        if not assignment:
    ...
    @staticmethod
    def get_team_mentor(db: Session, team_id: UUID) -> Mentor | None:
        ...
        if not assignment:
            return None
        return db.query(Mentor).filter(Mentor.id == assignment.mentor_id).first()

    @staticmethod
    def get_mentor_teams(db: Session, mentor_id: UUID) -> list[MentorTeamOut]:
    ...
    @staticmethod
    def list_assignments(db: Session, active_only: bool = True) -> list[dict]:
    ...
    @staticmethod
    def create_session(db: Session, mentor_id: UUID, data: MentorSessionCreate) -> MentorSession:
        # Validate mentor is assigned to team
        assignment = db.query(MentorAssignment).filter(
            MentorAssignment.mentor_id == mentor_id,
            MentorAssignment.team_id == data.team_id,
            MentorAssignment.is_active == True,
    ...
    @staticmethod
    def update_session(db: Session, mentor_id: UUID, session_id: UUID, data: MentorSessionUpdate) -> MentorSession:
        session = db.query(MentorSession).filter(
            MentorSession.id == session_id,
            MentorSession.mentor_id == mentor_id,
        ).first()
        if not session:
            raise HTTPException(status_code=404, detail="Session not found or not owned by this mentor.")
        update_data = data.model_dump(exclude_unset=True)
        for key, value in update_data.items():
    ...
    @staticmethod
    def list_sessions_for_team(db: Session, team_id: UUID) -> list[MentorSession]:
        return db.query(MentorSession).filter(
            MentorSession.team_id == team_id
        ).order_by(MentorSession.scheduled_at.desc()).all()

    @staticmethod
    def list_sessions_for_mentor(db: Session, mentor_id: UUID) -> list[MentorSession]:
        return db.query(MentorSession).filter(
            MentorSession.mentor_id == mentor_id
        ).order_by(MentorSession.scheduled_at.desc()).all()

    @staticmethod
    def get_next_session_for_team(db: Session, team_id: UUID) -> MentorSession | None:
        now = datetime.now(timezone.utc)
        return db.query(MentorSession).filter(
            MentorSession.team_id == team_id,
            MentorSession.status == "scheduled",
    ...
    @staticmethod
    def submit_team_feedback(db: Session, mentor_id: UUID, data: MentorFeedbackCreate) -> MentorFeedback:
        ...
        # If participant_id is given, validate they belong to the team
        if data.participant_id:
            ...
            if not participant:
                raise HTTPException(
                    status_code=400,
                    detail="Participant does not belong to this team."
                )
        db.add(feedback)
        db.commit()
        db.refresh(feedback)
    ...
    @staticmethod
    def get_visible_feedback_for_participant(db: Session, participant_id: UUID, team_id: UUID) -> list[MentorFeedback]:
        """Only feedback where visible_to_participant=True."""
        return db.query(MentorFeedback).filter(
            MentorFeedback.team_id == team_id,
            MentorFeedback.visible_to_participant == True,
            # Include team-level and individual-level for this participant
            (
                (MentorFeedback.participant_id == None) |
    ...
    @staticmethod
    def get_feedback_for_team(db: Session, team_id: UUID) -> list[MentorFeedback]:
        """All feedback for admin/mentor view."""
        return db.query(MentorFeedback).filter(
            MentorFeedback.team_id == team_id,
        ).order_by(MentorFeedback.created_at.desc()).all()

    @staticmethod
    def get_team_action_items(db: Session, team_id: UUID, visible_only: bool = False) -> list[str]:
        query = db.query(MentorFeedback).filter(MentorFeedback.team_id == team_id)
        if visible_only:
    ...
    @staticmethod
    def get_mentor_portal_me(db: Session, mentor_id: UUID) -> MentorPortalMe:
        mentor = MentorService.get_mentor(db, mentor_id)
        now = datetime.now(timezone.utc)
        ...
        # Teams needing daily update (assigned teams without feedback today)
    ...
    @staticmethod
    def get_participant_mentor_info(db: Session, participant_id: UUID, team_id: UUID) -> ParticipantMentorInfo:
        """Safe mentor data for participants — no private feedback."""
        mentor = MentorService.get_team_mentor(db, team_id)
        if not mentor:
            return ParticipantMentorInfo()

        next_meeting = MentorService.get_next_session_for_team(db, team_id)
        visible_feedback = MentorService.get_visible_feedback_for_participant(
            db, participant_id, team_id
        )
        action_items = MentorService.get_team_action_items(db, team_id, visible_only=True)
